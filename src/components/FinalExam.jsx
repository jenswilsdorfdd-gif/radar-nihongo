...
const FinalExam = ({ onBack, language }) => {
  ...
  // MONSTER-POOL: Vollständig lokalisierte Optionen
  const masterPool = [
    // --- KANA ---
  ...
  // Extrahiert den Text einer Option basierend auf der aktuellen Sprache
  ...
  const startExam = () => {
    ...
    const selected = shuffledPool.slice(0, 30).map(q => {
      ...
      const newCorrectIndex = shuffledOptions.indexOf(correctOptObj);
  ...
  const handleNext = () => {
    ...
    if (isCorrect) {
      ...
      setWrongAnswers(prev => [...prev, {
        ...
        userChoice: getOptionText(currentQ.options[selectedAnswer]),
        correctChoice: getOptionText(currentQ.options[currentQ.correct])
      }]);
    }
