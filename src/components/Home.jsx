...
const Home = ({ onSelectMode, onReset, onGoToWelcome, kanaReadDay, kanaWriteDay, readingDay, radarDay, kanjiDay, language }) => {
  ...
  // NEUE STATES FÜR DIE REGISTRIERUNG
  ...
  const texts = {
    de: {
      ...
      successText: "Du erhältst in Kürze eine Nachricht mit weitergehenden Informationen.",
      closeBtn: "Schließen"
    },
    en: {
      reset: "Reset",
      ...
      successText: "You will receive a message with further information shortly.",
      closeBtn: "Close"
    }
  };
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    // Hier werden die Daten später ans Backend/E-Mail gesendet
    console.log("Registrierungsdaten gesammelt:", formData);
    setIsSubmitted(true);
  };

  const closeRegistration = () => {
    setShowRegModal(false);
    // Kurze Verzögerung, damit die Animation beim Schließen sauber aussieht
    setTimeout(() => {
  ...
  return (
    <div ref={containerRef} className="flex-1 bg-gray-900 flex flex-col items-center p-6 text-white min-h-screen relative overflow-y-auto scrollbar-hide">
      ...
      {/* KONTROLLEN OBEN RECHTS */}
      <div className="absolute top-6 right-6 z-10 flex gap-4 items-center">
        <button 
          onClick={() => setShowInfoModal(true)} 
          className="text-cyan-400 hover:text-cyan-300 text-xs font-bold tracking-widest uppercase flex items-center gap-1 bg-cyan-900/30 px-3 py-1.5 rounded-full border border-cyan-500/50 transition-all active:scale-95"
        >
          <span>ℹ️</span> {t.info}
        </button>
      ...
      <button 
        ...
        className="mt-16 mb-10 flex flex-col items-center group cursor-pointer transition-transform active:scale-95 focus:outline-none"
      >
      ...
      <div className="w-full max-w-sm space-y-4 pb-12">
        ...
        <button onClick={() => onSelectMode('reading')} className="w-full bg-gray-800 hover:bg-gray-750 p-6 rounded-2xl border border-gray-700 hover:border-cyan-500/50 transition-all group text-left relative overflow-hidden">
          ...
          <div className="w-full bg-gray-700 h-1.5 rounded-full overflow-hidden">
            <div className="bg-cyan-500 h-full transition-all duration-500" style={{ width: `${(readingDay / 21) * 100}%` }}></div>
          </div>
        </button>
        ...
        {/* GRUPPEN-TRAINING REGISTRIERUNG */}
        <div className="w-full bg-gradient-to-br from-pink-900/40 to-purple-900/40 p-6 rounded-2xl border-2 border-pink-500/50 shadow-lg shadow-pink-500/20 text-center relative overflow-hidden">
          ...
          <button 
            ...
            className="w-full py-4 bg-pink-600 hover:bg-pink-500 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-all uppercase tracking-widest relative z-10"
          >
            {t.groupBtn}
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-gray-800 rounded-3xl border border-gray-600 shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <p className="text-gray-300 text-sm leading-relaxed italic border-l-4 border-cyan-500 pl-3">
                {t.modalIntro}
              </p>
              <div>
                <h3 className="font-bold text-green-400 mb-1">{t.modalW1Title}</h3>
                <p className="text-gray-400 text-sm leading-relaxed">{t.modalW1Desc}</p>
              </div>
              <div>
                <h3 className="font-bold text-yellow-400 mb-1">{t.modalW3Title}</h3>
                <p className="text-gray-400 text-sm leading-relaxed">{t.modalW3Desc}</p>
              </div>
              <div>
                <h3 className="font-bold text-purple-400 mb-1">{t.modalW6Title}</h3>
                <p className="text-gray-400 text-sm leading-relaxed">{t.modalW6Desc}</p>
              </div>
            </div>

            <div className="p-4 bg-gray-900 border-t border-gray-700 shrink-0">
              <button onClick={() => setShowInfoModal(false)} className="w-full py-4 bg-cyan-600 hover:bg-cyan-500 rounded-xl font-bold text-white tracking-widest uppercase transition-colors">
                {t.modalClose}
              </button>
            </div>
          </div>
      ...
      {/* ---------------------------------------------------- */}
      {/* 2. REGISTRIERUNGS MODAL (Live Dojo)                  */}
      {/* ---------------------------------------------------- */}
      {showRegModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md bg-gray-800 rounded-3xl border border-pink-500/50 shadow-[0_0_30px_rgba(236,72,153,0.2)] overflow-hidden flex flex-col">
            ...
            <div className="p-6 bg-gray-900 border-b border-gray-700 flex justify-between items-center">
              ...
              {!isSubmitted && (
                <button onClick={closeRegistration} className="text-gray-400 hover:text-white text-3xl leading-none">&times;</button>
              )}
            </div>
              ...
              {!isSubmitted ? (
                <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div className="pt-4 flex gap-3">
                    <button type="button" onClick={closeRegistration} className="flex-1 py-4 bg-gray-700 hover:bg-gray-600 rounded-xl font-bold text-white uppercase tracking-widest text-xs transition-colors">
                      {t.cancelBtn}
                    </button>
                    <button type="submit" className="flex-[2] py-4 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 rounded-xl font-bold text-white uppercase tracking-widest text-xs shadow-lg active:scale-95 transition-all">
                      {t.submitBtn}
                    </button>
                  </div>
                </form>
                ...
                <div className="flex flex-col items-center justify-center py-6 text-center animate-fade-in">
                  <div className="w-20 h-20 bg-pink-900/30 border-2 border-pink-500 rounded-full flex items-center justify-center text-4xl mb-6 shadow-[0_0_30px_rgba(236,72,153,0.4)]">
                    ✓
                  </div>
                  <h2 className="text-2xl font-bold text-white mb-2">{t.successTitle}</h2>
                  <p className="text-gray-400 mb-8">{t.successText}</p>
                  <button onClick={closeRegistration} className="w-full py-4 bg-gray-700 hover:bg-gray-600 rounded-xl font-bold text-white uppercase tracking-widest transition-all active:scale-95">
                    {t.closeBtn}
                  </button>
                </div>
              )}
